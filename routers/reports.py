...
def get_area_name(lat: float, lon: float) -> str:
    """Reverse geocode GPS to neighborhood/suburb name."""
    try:
...
async def analyze_image(file: UploadFile = File(...), user: dict = Depends(get_current_user)):
    """
    Real-time AI analysis endpoint for direct image uploads.
    Returns detected damage types and their severity.
    """
    try:
        content = await file.read()
        result = analyze_road_damage(content)
        return result
...
class ReportCreate(BaseModel):
    ...
    is_impact_verified: Optional[bool] = False
...
async def create_report(report: ReportCreate, db: Session = Depends(get_db), user: dict = Depends(get_current_user)):
    ...
    # Sentinel Elite Logic
    db_report.source = report.source
    db_report.is_impact_verified = report.is_impact_verified
    
    # Auto-map to Area if not provided
    if not report.area_name:
        db_report.area_name = get_area_name(report.location.latitude, report.location.longitude)
    else:
        db_report.area_name = report.area_name
    ...
    # 1. Deterioration Check (Compare with historical reports in 10m radius)
    ...
    if historical:
        ...
        if curr_sev > prev_sev:
            await manager.broadcast({
                ...
                "area": db_report.area_name,
                "growth": f"{((curr_sev - prev_sev) / prev_sev * 100):.0f}% increase",
                "report_id": db_report.report_id
            })

    # 2. Live Warning Broadcast (To other drivers using the app)
    if (db_report.ai_suggested_severity or 0) >= 4 or db_report.is_impact_verified:
        await manager.broadcast({
            "type": "HAZARD_ALERT",
            "hazard_type": db_report.damage_type,
            "severity": db_report.ai_suggested_severity or 5,
            "lat": db_report.latitude,
            "lon": db_report.longitude,
            "msg": f"CAUTION: {db_report.damage_type} detected ahead!"
        })
    # ──────────────────────────────────────────────────────────────────────
